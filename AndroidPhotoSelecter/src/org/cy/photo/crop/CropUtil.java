...
public class CropUtil {
	...
	@Nullable
	private static File getFromMediaUriPfd(Context context,
			ContentResolver resolver, Uri uri) {
		if (uri == null)
			return null;
	...
	public static void startBackgroundJob(ProgressDialog dialog, Runnable job) {
		// Make the progress dialog uncancelable, so that we can guarantee
		// the thread will be done before the activity getting destroyed
		// ProgressDialog dialog = ProgressDialog.show(activity, title, message,
		// true, false);
		new Thread(new BackgroupJob(job, dialog)).start();
	}

	private static class BackgroupJob implements Runnable {
		private final ProgressDialog dialog;
		private final Runnable job;

		public BackgroupJob(Runnable job, ProgressDialog dialog) {
			this.dialog = dialog;
			this.job = job;
		}
